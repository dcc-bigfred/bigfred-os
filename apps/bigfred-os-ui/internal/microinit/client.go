// Package microinit talks to the microinit PID 1 control socket.
package microinit
...
func (c *Client) Control(name, action string) error {
	...
	var resp Response
	if err := c.roundTrip(request{Type: typ, Name: name}, &resp); err != nil {
		return err
	}
	switch resp.Type {
	case "ok":
...
// FormatLogLine renders a LogLine for the UI console.
