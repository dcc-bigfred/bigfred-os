...
macro_rules! logf {
    ($($t:tt)*) => {{
        let _ = writeln!(::std::io::stderr(), "prepare-nvme: {}", format_args!($($t)*));
    }};
}

const DATA_MOUNT: &str = "/data";
const TMP_MOUNT: &str = "/run/prepare-nvme";
...
const NVME_MARKER: &str = ".bigfred-nvme";
/// Marker directory shipped on BigFred OS images (see post-build.sh).
const BIGFRED_MARKER: &str = "/var/lib/bigfred";

const MKE2FS_BIN: &str = "/sbin/mke2fs";
const MKFS_EXT4_BIN: &str = "/sbin/mkfs.ext4";
const BLKID_BIN: &str = "/sbin/blkid";
const PARTPROBE_BIN: &str = "/usr/sbin/partprobe";
const MOUNT_BIN: &str = "/bin/mount";
const UMOUNT_BIN: &str = "/bin/umount";
const CP_BIN: &str = "/bin/cp";
const FUSER_BIN: &str = "/bin/fuser";
...
fn create_single_partition(disk: &str) -> Result<()> {
...
fn find_nvme_disk() -> Result<Option<String>> {
...
fn list_partitions(disk: &str) -> Result<Vec<String>> {
...
fn sort_partitions(parts: &mut [String], disk_base: &str) {
    let prefix = format!("/dev/{disk_base}p");
    parts.sort_by_key(|p| partition_index(p, &prefix));
}

fn partition_index(path: &str, prefix: &str) -> u32 {
...
fn wait_for_partition(disk: &str, timeout: Duration) -> Result<String> {
    ...
    while Instant::now() < deadline {
        ...
        let base = Path::new(disk).file_name().and_then(|s| s.to_str()).unwrap_or("?");
...
fn format_ext4(part: &str) -> Result<()> {
...
fn blkid_type(dev: &str) -> Result<String> {
...
fn partition_looks_empty(part: &str) -> Result<bool> {
    let dir = tempfile_dir("prepare-nvme-check-")?;
    let _cleanup = DirGuard(dir.clone());

    logf!("emptiness probe: mount -t ext4 -o ro {part} {}", dir.display());
...
fn dir_empty_except_lost_found(dir: &Path) -> Result<bool> {
...
fn is_mounted(dir: &str) -> bool {
    !mount_source(dir).is_empty()
}

fn mount_source(dir: &str) -> String {
...
fn describe_mount(dir: &str) -> String {
    let Ok(content) = fs::read_to_string("/proc/mounts") else {
...
fn format_cmd_out(out: &str) -> String {
    if out.is_empty() {
        return String::new();
    }
