#!/usr/bin/env bash
# Pull microdns linux/arm64 OCI bundle from GHCR and write a tar for Buildroot.
# Usage: fetch-oci.sh <oci-tag> <output.tar>
#
# Env:
#   MICRODNS_OCI_IMAGE — default ghcr.io/dcc-bigfred/microdns-linux-arm64
...
BIN="${tmpdir}/microdns-linux-arm64"
if [[ ! -f "${BIN}" ]]; then
	echo "error: expected microdns-linux-arm64 in OCI layer, found:" >&2
...
chmod 755 "${BIN}"
mkdir -p "$(dirname "${OUT}")"
# Stable layout for EXTRACT/INSTALL: bin/microdns
staged="${tmpdir}/stage"
mkdir -p "${staged}/bin"
cp -f "${BIN}" "${staged}/bin/microdns"
